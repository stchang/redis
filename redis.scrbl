...
@author[@author+email["Marc Burns" "m4burns@uwaterloo.ca"]]
...
@defproc[(make-connection-pool
          ...
          [#:max-connections max-conn exact-nonnegative-integer? 100]
          [#:max-idle max-idle exact-nonnegative-integer? 10])
         redis-connection-pool?]{
  Makes a Redis connection pool with the given parameters.

  A connection pool manages leasing out connections to threads and
maintains a set of idle redis connections. Calling
@racket[connection-pool-lease] will return a new connection, either by
removing it from the set of idle connections or by establishing a fresh
connection to the redis server.

  A thread can lease only one connection at a time. Subsequent calls to
@racket[connection-pool-lease] from the same thread will return the same
connection until @racket[connection-pool-return] is called. When
@racket[connection-pool-return] is called, the connection may be closed,
added to the idle set, or leased immediately to a different thread.

  If a thread exits or is killed before returning a leased connection,
the connection is automatically returned to the pool from which it was
leased.

  New connections are made to @racket[host]:@racket[port]. Connections will
be closed shortly after they are returned when the idle set contains
@racket[max-idle] connections. A thread will receive an exception if
attempting to lease a connection when the pool is managing @racket[max-conn]
connections and the idle set is empty.}
...
used if no pool is given.
  ...
  The pool maintains a map of threads to connections: If a connection
already exists for the current thread, then that connection is returned.
...
used if no pool is given.
  ...
  After calling @racket[connection-pool-return], the connection may be closed
or leased to another thread.

  UNSUBSCRIBE, PUNSUBSCRIBE, and UNWATCH will be called on the connection before it
is leased to another thread.

  Returning an already returned connection or attempting to use a connection
after it has been returned will raise an exception.}
...
@itemlist[
          @item{The following functions are available:
                APPEND DEL GETSET MGET SETRANGE STRLEN EXISTS MSETNX RENAMENX
                GET GETRANGE RANDOMKEY SET MSET RENAME DUMP RESTORE SETBIT
                GETBIT BITCOUNT BITOP LPUSH LPUSHX LRANGE LLEN LREM RPUSH
                RPUSHX LINDEX LPOP RPOP RPOPLPUSH BLPOP BRPOP BRPOPL PUSH
                LINSERT LSET LTRIM HDEL HGETALL HINCRBY HINCRBYFLOAT HMGET
                HVALS HMSET HSET HEXISTS HSETNX HGET SADD SCARD SDIFF SDIFFSTORE
                SINTER SINTERSTORE SMEMBERS SREM SUNION SUNIONSTORE SISMEMBER
                SMOVE SPOP SRANDMEMBER ZADD ZCARD ZCOUNT ZINCRBY ZINTERSTORE
                ZRANGE ZRANGEBYSCORE ZREM ZREMRANGEBYRANK ZREMRANGEBYSCORE
                ZREVRANGE ZREVRANGEBYSCORE ZUNIONSTORE ZRANK ZREVRANK ZSCORE DECR
                DECRBY INCR INCRBY INCRBYFLOAT HKEYS HLEN EXPIRE EXPIREAT PERSIST
                PEXPIRE PEXPIREAT TTL PTTL MULTI DISCARD WATCH UNWATCH EXEC
                SUBSCRIBE UNSUBSCRIBE PSUBSCRIBE PUNSUBSCRIBE PUBLISH DBSIZE ECHO
                FLUSHALL FLUSHDB KEYS PING TIME TYPE AUTH QUIT SELECT}
           ...
           @item{The following SET variations, which allow different types of input, are defined:
                 SET/list SET/set SET/hash SET/heap}

           @item{The following hash variations are defined: HGET/str}

           @item{The following POP variations are defined: POP/list}

           @item{@racket[(do-MULTI c ...)] wraps @racket[MULTI] and
                  @racket[EXEC] around the given commands.}

           @item{The following functions are not currently available:
                 @itemlist[
                           @item{SETNX, SETEX, PSETEX (use SET + options instead)}
                            @item{BGREWRITEAOF BGSAVE INFO LASTSAVE MIGRATE MONITOR MOVE SAVE
                                  SHUTDOWN SLAVEOF SLOWLOG SYNC CLIENT CONFIG DEBUG EVAL EVALSHA
                                  SCRIPT}
                                         ]}
                                           ]
...
subscription messages are sent.
  ...
  To (p)unsubscribe from a subscription, use @racket[send-cmd] or @racket[UNSUBSCRIBE],
or @racket[PUNSUBSCRIBE] with the appropriate @tech{pubsub-specific connection}.
