#lang racket
(require "redis.rkt" "redis-cmds.rkt" "redis-error.rkt" "test-utils.rkt")
(require racket/async-channel)

;; connection pool concurrency tests

(module+ main (displayln "Run tests with \"raco test pool-tests.rkt\"."))
...
(module+ test
  ...
  (define-test-suite pool-tests
    (test-begin
     ;; check that owner is properly set if connection 
     ;; is released-then-leased quickly
     ...
     ;; (loop test below checks for starvation)
     (check-eq? conn conn2)
     ...
     (check-not-exn
       (lambda ()
         (let loop ([n 10000]
           ...
           (if (zero? n)
               (for ([conn s]) (connection-pool-return conn p))
               (begin
                 (connection-pool-return (set-first s) p)
                 (loop (sub1 n)
                       (set-add (set-rest s) (connection-pool-lease p))))))))

     ;; check that connections established in separate custodian are not closed
         ...
         (lambda ()
           (define threads
             (for/list ([n (in-range 1 50)])
               (thread
                 (thunk
                  (for/list ([k (in-range (random 2000))])
                    (let ([c (random 100)])
                      (unless
                        (= c
                           (string->number
     ...
     (sleep 1) ; need this ow program shutdown begins and checks get ignored
